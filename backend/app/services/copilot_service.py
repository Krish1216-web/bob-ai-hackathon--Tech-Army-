...
"""
        ...
        if (provider in ["gemini", "auto"]) and gemini_key:
            models = ["models/gemini-flash-latest", "gemini-flash-latest", "models/gemini-pro-latest", "models/gemma-4-26b-a4b-it"]
            for model in models:
                try:
                    m_path = model if model.startswith("models/") else f"models/{model}"
                    gemini_url = f"https://generativelanguage.googleapis.com/v1beta/{m_path}:generateContent?key={gemini_key}"
                    gemini_payload = {
                        "contents": [
                            {
                                "parts": [
                        ...
                        if candidates:
                            ...
                            if parts and "text" in parts[0]:
                                ...
                                return CopilotQueryResponse(
                                    ...
                                    answer=generated,
                                    confidence=0.99,
                                    sources=[f"Google {model.replace('models/', '').title()} (Live LLM)", "Live PostgreSQL/SQLite RAG", "IoT Sensor Stream"],
